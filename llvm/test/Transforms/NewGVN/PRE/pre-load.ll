...
; CHECK-SAME: ptr [[P:%.*]], ptr [[Q:%.*]], ptr [[HACK:%.*]], i1 [[C:%.*]]) {
; CHECK-NEXT:  block1:
...
; CHECK-NEXT:    br label [[BLOCK4]]
; CHECK:       block4:
; CHECK-NEXT:    [[PHIOFOPS1:%.*]] = phi ptr [ [[A]], [[BLOCK3]] ], [ [[B]], [[BLOCK2]] ]
; CHECK-NEXT:    [[PRE:%.*]] = phi i32 [ 0, [[BLOCK3]] ], [ [[PRE_PRE]], [[BLOCK2]] ]
; CHECK-NEXT:    [[P2:%.*]] = phi ptr [ [[P]], [[BLOCK3]] ], [ [[Q]], [[BLOCK2]] ]
; CHECK-NEXT:    ret i32 [[PRE]]
;
...
; CHECK-NEXT:    br label [[BB:%.*]]
; CHECK:       bb:
; CHECK-NEXT:    [[PHIOFOPS1:%.*]] = phi ptr [ [[G]], [[BB_NPH]] ], [ [[SCEVGEP:%.*]], [[BB]] ]
...
; CHECK-NEXT:    [[TMP9]] = add i64 [[INDVAR]], 1
; CHECK-NEXT:    [[SCEVGEP10:%.*]] = getelementptr double, ptr [[G]], i64 [[TMP9]]
; CHECK-NEXT:    [[TMP2:%.*]] = load double, ptr [[SCEVGEP10]], align 8
; CHECK-NEXT:    [[TMP3:%.*]] = load double, ptr [[SCEVGEP]], align 8
; CHECK-NEXT:    [[TMP4:%.*]] = fadd double [[TMP2]], [[TMP3]]
...
; CHECK-NEXT:    br label [[BB:%.*]]
; CHECK:       bb:
; CHECK-NEXT:    [[PHIOFOPS1:%.*]] = phi ptr [ [[G]], [[BB_NPH]] ], [ [[SCEVGEP12:%.*]], [[BB]] ]
; CHECK-NEXT:    [[TMP2:%.*]] = phi double [ [[DOTPRE]], [[BB_NPH]] ], [ [[TMP6:%.*]], [[BB]] ]
...
; CHECK:       block3:
; CHECK-NEXT:    store i32 0, ptr [[P]], align 4
; CHECK-NEXT:    br label [[BLOCK2_BLOCK4_CRIT_EDGE]]
; CHECK:       block4:
; CHECK-NEXT:    [[PRE:%.*]] = load i32, ptr [[P]], align 4
; CHECK-NEXT:    br label [[BLOCK5]]
; CHECK:       block5:
; CHECK-NEXT:    [[RET:%.*]] = phi i32 [ 0, [[BLOCK2]] ], [ [[PRE]], [[BLOCK2_BLOCK4_CRIT_EDGE]] ]
...
; CHECK-SAME: ptr [[P:%.*]]) personality ptr @__CxxFrameHandler3 {
; CHECK-NEXT:  block1:
...
; CHECK-SAME: ptr noalias nocapture readonly [[X:%.*]], ptr noalias nocapture [[R:%.*]], i32 [[A:%.*]]) {
; CHECK-NEXT:  entry:
...
; CHECK-SAME: ptr noalias nocapture readonly align 4 dereferenceable(8) [[X:%.*]], ptr noalias nocapture [[R:%.*]], i32 [[A:%.*]]) #[[ATTR1:[0-9]+]] {
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[TOBOOL:%.*]] = icmp eq i32 [[A]], 0
; CHECK-NEXT:    br i1 [[TOBOOL]], label [[IF_END:%.*]], label [[IF_THEN:%.*]]
; CHECK:       if.then:
...
; CHECK-SAME: ptr noalias nocapture readonly align 4 dereferenceable(8) [[X:%.*]], ptr noalias nocapture [[R:%.*]], i32 [[A:%.*]]) #[[ATTR1]] {
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[TOBOOL:%.*]] = icmp eq i32 [[A]], 0
; CHECK-NEXT:    br i1 [[TOBOOL]], label [[IF_END:%.*]], label [[IF_THEN:%.*]]
; CHECK:       if.then:
...
; CHECK-SAME: ptr [[P1:%.*]], ptr [[P2:%.*]], ptr [[P3:%.*]], ptr [[P4:%.*]]) {
; CHECK-NEXT:  entry:
...
; CHECK-SAME: i1 [[COND:%.*]], ptr [[P1:%.*]], ptr [[P2:%.*]]) personality ptr @__CxxFrameHandler3 {
; CHECK-NEXT:  entry:
...
; CHECK:       else:
; CHECK-NEXT:    invoke void @f()
; CHECK-NEXT:            to label [[END]] unwind label [[LPAD:%.*]]
; CHECK:       end:
; CHECK-NEXT:    [[V1:%.*]] = load i64, ptr [[P1]], align 8
...
; CHECK-SAME: i1 [[COND:%.*]], i1 [[COND2:%.*]], ptr [[P1:%.*]], ptr [[P2:%.*]]) {
; CHECK-NEXT:  entry:
...
; CHECK-SAME: i1 [[COND:%.*]], i32 [[CODE:%.*]], ptr [[P1:%.*]], ptr [[P2:%.*]]) {
; CHECK-NEXT:  entry:
; CHECK-NEXT:    br i1 [[COND]], label [[IF_THEN:%.*]], label [[IF_ELSE:%.*]]
; CHECK:       if.then:
; CHECK-NEXT:    [[V1:%.*]] = load i16, ptr [[P1]], align 2
...
; CHECK-NEXT:    br label [[IF_END:%.*]]
; CHECK:       if.end:
; CHECK-NEXT:    [[V2:%.*]] = load i64, ptr [[P1]], align 8
; CHECK-NEXT:    store i64 [[V2]], ptr [[P2]], align 8
; CHECK-NEXT:    ret void
; CHECK:       if.else:
; CHECK-NEXT:    [[COND2:%.*]] = call i1 @foo()
; CHECK-NEXT:    br i1 [[COND2]], label [[IF_END]], label [[EXIT:%.*]]
; CHECK:       exit:
...
; CHECK:       then:
; CHECK-NEXT:    [[VAL2:%.*]] = load i64, ptr @B, align 8
