...
; CHECK-LABEL: define i32 @_ZNK21mrZEllipticalCylinder10viewingHitERK6ggRay3dddR18mrViewingHitRecordR16ggMaterialRecord(
; CHECK-SAME: ptr [[THIS:%.*]], ptr [[RAY:%.*]], double [[UNNAMED_ARG:%.*]], double [[TMIN:%.*]], double [[TMAX:%.*]], ptr [[VHR:%.*]], ptr [[UNNAMED_ARG2:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*:]]
; CHECK-NEXT:    [[TMP80_I:%.*]] = getelementptr [[STRUCT_MRVIEWINGHITRECORD:%.*]], ptr [[VHR]], i32 0, i32 1, i32 0, i32 0
; CHECK-NEXT:    store double 0.000000e+00, ptr [[TMP80_I]], align 8
; CHECK-NEXT:    br i1 false, label %[[ENTRY_RETURN_CRIT_EDGE:.*]], label %[[COND_NEXT_I:.*]]
; CHECK:       [[ENTRY_RETURN_CRIT_EDGE]]:
; CHECK-NEXT:    br label %[[RETURN:.*]]
; CHECK:       [[COND_NEXT_I]]:
; CHECK-NEXT:    br i1 false, label %[[COND_NEXT_I_RETURN_CRIT_EDGE:.*]], label %[[COND_TRUE:.*]]
; CHECK:       [[COND_NEXT_I_RETURN_CRIT_EDGE]]:
; CHECK-NEXT:    br label %[[RETURN]]
; CHECK:       [[COND_TRUE]]:
