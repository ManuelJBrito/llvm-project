...
; CHECK:       [[BB0]]:
; CHECK-NEXT:    [[X_TR:%.*]] = phi ptr [ [[X]], %[[ENTRY]] ], [ null, %[[LAND_LHS_TRUE_BB0_CRIT_EDGE:.*]] ]
; CHECK-NEXT:    [[TMP0:%.*]] = load i16, ptr [[X_TR]], align 4
; CHECK-NEXT:    [[CONV:%.*]] = zext i16 [[TMP0]] to i32
; CHECK-NEXT:    switch i32 [[CONV]], label %[[IF_END_50:.*]] [
; CHECK-NEXT:      i32 43, label %[[BB0_CLEANUP_CRIT_EDGE:.*]]
; CHECK-NEXT:      i32 52, label %[[IF_THEN_5:.*]]
; CHECK-NEXT:    ]
; CHECK:       [[BB0_CLEANUP_CRIT_EDGE]]:
; CHECK-NEXT:    br label %[[CLEANUP:.*]]
; CHECK:       [[IF_THEN_5]]:
; CHECK-NEXT:    br i1 undef, label %[[LAND_LHS_TRUE:.*]], label %[[IF_THEN_26:.*]]
; CHECK:       [[LAND_LHS_TRUE]]:
; CHECK-NEXT:    br i1 undef, label %[[LAND_LHS_TRUE_CLEANUP_CRIT_EDGE:.*]], label %[[LAND_LHS_TRUE_BB0_CRIT_EDGE]]
; CHECK:       [[LAND_LHS_TRUE_BB0_CRIT_EDGE]]:
; CHECK-NEXT:    br label %[[BB0]]
; CHECK:       [[LAND_LHS_TRUE_CLEANUP_CRIT_EDGE]]:
; CHECK-NEXT:    br label %[[CLEANUP]]
; CHECK:       [[IF_THEN_26]]:
; CHECK-NEXT:    br i1 undef, label %[[IF_THEN_26_COND_END_CRIT_EDGE:.*]], label %[[COND_FALSE:.*]]
; CHECK:       [[IF_THEN_26_COND_END_CRIT_EDGE]]:
...
; CHECK-NEXT:    br label %[[COND_END]]
; CHECK:       [[COND_END]]:
; CHECK-NEXT:    br i1 undef, label %[[IF_THEN_44:.*]], label %[[COND_END_CLEANUP_CRIT_EDGE:.*]]
; CHECK:       [[COND_END_CLEANUP_CRIT_EDGE]]:
; CHECK-NEXT:    br label %[[CLEANUP]]
; CHECK:       [[IF_THEN_44]]:
...
; CHECK-NEXT:    br i1 undef, label %[[FOR_BODY_57:.*]], label %[[IF_END_50_CLEANUP_CRIT_EDGE:.*]]
; CHECK:       [[IF_END_50_CLEANUP_CRIT_EDGE]]:
; CHECK-NEXT:    br label %[[CLEANUP]]
; CHECK:       [[FOR_BODY_57]]:
; CHECK-NEXT:    unreachable
; CHECK:       [[CLEANUP]]:
...
; CHECK-NEXT:    br i1 undef, label %[[WHILE_BODYTHREAD_PRE_SPLIT_IF_THEN_14_CRIT_EDGE:.*]], label %[[WHILE_BODYTHREAD_PRE_SPLIT_IF_END_15_CRIT_EDGE:.*]]
...
; CHECK:       [[IF_THEN_14]]:
; CHECK-NEXT:    [[PHIOFOPS:%.*]] = phi i32 [ [[V1_PRE]], %[[WHILE_BODYTHREAD_PRE_SPLIT_IF_THEN_14_CRIT_EDGE]] ], [ [[SUB_PTR_RHS_CAST25:%.*]], %[[WHILE_END_IF_THEN_14_CRIT_EDGE:.*]] ]
; CHECK-NEXT:    br label %[[IF_END_15]]
; CHECK:       [[IF_END_15]]:
; CHECK-NEXT:    [[V2:%.*]] = load ptr, ptr @yy_c_buf_p, align 4
; CHECK-NEXT:    br label %[[WHILE_COND_16:.*]]
; CHECK:       [[WHILE_COND_16]]:
; CHECK-NEXT:    br i1 undef, label %[[WHILE_COND_16_WHILE_COND_16_CRIT_EDGE:.*]], label %[[WHILE_END:.*]]
; CHECK:       [[WHILE_COND_16_WHILE_COND_16_CRIT_EDGE]]:
; CHECK-NEXT:    br label %[[WHILE_COND_16]]
; CHECK:       [[WHILE_END]]:
...
; CHECK-NEXT:    br label %[[IF_THEN_14]]
; CHECK:       [[WHILE_END_WHILE_BODYTHREAD_PRE_SPLIT_CRIT_EDGE1]]:
; CHECK-NEXT:    br label %[[WHILE_BODYTHREAD_PRE_SPLIT]]
; CHECK:       [[WHILE_END_WHILE_BODYTHREAD_PRE_SPLIT_CRIT_EDGE]]:
; CHECK-NEXT:    br label %[[WHILE_BODYTHREAD_PRE_SPLIT]]
; CHECK:       [[SW_DEFAULT]]:
; CHECK-NEXT:    unreachable
; CHECK:       [[RETURN]]:
