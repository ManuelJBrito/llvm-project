...
; RUN: opt -passes=newgvn -enable-split-backedge-in-load-pre -S < %s | FileCheck %s
...
; CHECK-SAME: ptr noalias captures(none) [[P:%.*]], ptr noalias captures(none) [[Q:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*:]]
; CHECK-NEXT:    [[X:%.*]] = load i32, ptr [[P]], align 4
; CHECK-NEXT:    [[TMP0:%.*]] = load volatile i32, ptr [[Q]], align 4
; CHECK-NEXT:    [[Y:%.*]] = load volatile i32, ptr [[P]], align 4
...
; CHECK-SAME: i1 [[C:%.*]], ptr noalias captures(none) [[P:%.*]], ptr noalias captures(none) [[Q:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*:]]
...
; CHECK-SAME: i1 [[B:%.*]], i1 [[C:%.*]], ptr noalias [[P:%.*]], ptr noalias [[Q:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*]]:
; CHECK-NEXT:    [[Y1:%.*]] = load i32, ptr [[P]], align 4
; CHECK-NEXT:    call void @use(i32 [[Y1]])
; CHECK-NEXT:    br label %[[HEADER:.*]]
