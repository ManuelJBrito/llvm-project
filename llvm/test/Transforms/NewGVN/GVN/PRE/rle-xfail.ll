...
; XFAIL: *
; RUN: opt < %s -data-layout="e-p:32:32:32-p1:16:16:16-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:32:64-f32:32:32-f64:32:64-n8:16:32" -passes=newgvn,dce -enable-split-backedge-in-load-pre -S | FileCheck %s --check-prefixes=CHECK,LE
; RUN: opt < %s -data-layout="E-p:32:32:32-p1:16:16:16-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:32:64-f32:32:32-f64:64:64-n32" -passes=newgvn,dce -enable-split-backedge-in-load-pre -S | FileCheck %s --check-prefixes=CHECK,BE
...
TX:
  ...
  ; implementation of newgvn would see that we're returning 17 if the loop
  ; executes once or 42 if it executes more than that, but we'd have to do
  ; loop restructuring to expose this, and newgvn shouldn't do this sort of CFG
  ; transformation.
...
; LE-LABEL: define i32 @load_load_partial_alias_cross_block_phi_trans(
; LE-SAME: ptr [[P:%.*]]) #[[ATTR3]] {
; LE-NEXT:  [[ENTRY:.*:]]
; LE-NEXT:    [[X1:%.*]] = load i32, ptr [[P]], align 4
; LE-NEXT:    [[CMP:%.*]] = icmp eq i32 [[X1]], 127
...
; LE-NEXT:    ret i32 52
;
; BE-LABEL: define i32 @load_load_partial_alias_cross_block_phi_trans(
; BE-SAME: ptr [[P:%.*]]) #[[ATTR3]] {
; BE-NEXT:  [[ENTRY:.*:]]
; BE-NEXT:    [[X1:%.*]] = load i32, ptr [[P]], align 4
; BE-NEXT:    [[CMP:%.*]] = icmp eq i32 [[X1]], 127
