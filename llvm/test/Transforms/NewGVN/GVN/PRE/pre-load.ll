...
; RUN: opt < %s -passes=newgvn -S | FileCheck %s --check-prefixes=CHECK,MDEP
; RUN: opt < %s -passes=newgvn -S | FileCheck %s --check-prefixes=CHECK,MSSA
...
; CHECK-NEXT:    br label [[BLOCK4]]
; CHECK:       block4:
; CHECK-NEXT:    [[PRE_PRE_PHI:%.*]] = phi i32 [ 0, [[BLOCK3]] ], [ [[PRE_PRE]], [[BLOCK2]] ]
; CHECK-NEXT:    ret i32 [[PRE_PRE_PHI]]
;
block1:
...
; CHECK-NEXT:    br label [[BLOCK4]]
; CHECK:       block4:
; CHECK-NEXT:    [[PRE_PRE_PHI:%.*]] = phi i32 [ 0, [[BLOCK3]] ], [ [[PRE_PRE]], [[BLOCK2_BLOCK4_CRIT_EDGE]] ]
; CHECK-NEXT:    br label [[BLOCK5]]
; CHECK:       block5:
; CHECK-NEXT:    [[RET:%.*]] = phi i32 [ 0, [[BLOCK2]] ], [ [[PRE_PRE_PHI]], [[BLOCK4]] ]
...
; CHECK-LABEL: @test18(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    br i1 [[COND:%.*]], label [[ENTRY_IF_END_CRIT_EDGE:%.*]], label [[IF_THEN:%.*]]
; CHECK:       entry.if.end_crit_edge:
...
; CHECK-NEXT:    br label [[IF_END]]
; CHECK:       if.end:
; CHECK-NEXT:    [[V2_PRE_PHI:%.*]] = phi i16 [ [[DEC]], [[IF_THEN]] ], [ [[V2_PRE]], [[ENTRY_IF_END_CRIT_EDGE]] ]
; CHECK-NEXT:    store i16 [[V2_PRE_PHI]], ptr [[P2:%.*]], align 2
; CHECK-NEXT:    ret void
;
entry:
...
; CHECK-NEXT:    br label [[END]]
; CHECK:       end:
; CHECK-NEXT:    [[V1_PRE_PHI:%.*]] = phi i64 [ [[V1_PRE]], [[ELSE_END_CRIT_EDGE]] ], [ [[ADD]], [[THEN]] ]
; CHECK-NEXT:    [[AND:%.*]] = and i64 [[V1_PRE_PHI]], 100
...
; CHECK-NEXT:    br label [[IF_END:%.*]]
; CHECK:       if.end:
; CHECK-NEXT:    [[V2_PRE_PHI:%.*]] = phi i64 [ [[V2_PRE:%.*]], [[IF_ELSE_IF_END_CRIT_EDGE:%.*]] ], [ [[DEC]], [[IF_THEN]] ]
; CHECK-NEXT:    store i64 [[V2_PRE_PHI]], ptr [[P2:%.*]], align 8
; CHECK-NEXT:    ret void
; CHECK:       if.else:
...
; CHECK-NEXT:    br i1 [[COND2:%.*]], label [[THEN:%.*]], label [[ENTRY_WRONG_CRIT_EDGE:%.*]]
; CHECK:       entry.wrong_crit_edge:
...
; CHECK-NEXT:    br label [[WRONG]]
; CHECK:       wrong:
; CHECK-NEXT:    [[VAL1_PRE_PHI:%.*]] = phi i64 [ [[VAL2]], [[STORE]] ], [ [[VAL1_PRE]], [[ENTRY_WRONG_CRIT_EDGE]] ]
; CHECK-NEXT:    store i64 [[VAL1_PRE_PHI]], ptr [[G]], align 8
; CHECK-NEXT:    ret void
; CHECK:       exit:
; CHECK-NEXT:    ret void
;
