...
; RUN: opt -passes=newgvn -S < %s | FileCheck %s --check-prefixes=CHECK,MDEP
; RUN: opt -passes=newgvn -S < %s | FileCheck %s --check-prefixes=CHECK,MSSA
...
; CHECK-NEXT:    [[J_PRE:%.*]] = sext i32 [[X]] to i64, !dbg [[DBG9:![0-9]+]]
; CHECK-NEXT:    [[Q_PHI_TRANS_INSERT:%.*]] = getelementptr [100 x i32], ptr @G, i64 0, i64 [[J_PRE]], !dbg [[DBG10:![0-9]+]]
; CHECK-NEXT:    [[N_PRE:%.*]] = load i32, ptr [[Q_PHI_TRANS_INSERT]], align 4, !dbg [[DBG11:![0-9]+]]
; CHECK-NEXT:    br label %[[END:.*]], !dbg [[DBG8]]
; CHECK:       [[THEN]]:
; CHECK-NEXT:    [[I:%.*]] = sext i32 [[X]] to i64, !dbg [[DBG12:![0-9]+]]
; CHECK-NEXT:    [[P:%.*]] = getelementptr [100 x i32], ptr @G, i64 0, i64 [[I]], !dbg [[DBG12]]
; CHECK-NEXT:    store i32 [[Z]], ptr [[P]], align 4, !dbg [[DBG12]]
; CHECK-NEXT:    br label %[[END]], !dbg [[DBG12]]
; CHECK:       [[END]]:
; CHECK-NEXT:    [[N_PRE_PHI:%.*]] = phi i32 [ [[N_PRE]], %[[ENTRY_END_CRIT_EDGE]] ], [ [[Z]], %[[THEN]] ]
; CHECK-NEXT:    [[PRE_PHI:%.*]] = phi i64 [ [[I]], %[[THEN]] ], [ [[J_PRE]], %[[ENTRY_END_CRIT_EDGE]] ]
; CHECK-NEXT:    [[Q:%.*]] = getelementptr [100 x i32], ptr @G, i64 0, i64 [[PRE_PHI]], !dbg [[DBG10]]
; CHECK-NEXT:    ret i32 [[N_PRE_PHI]], !dbg [[DBG11]]
;
entry:
...
; CHECK: [[DBG9]] = !DILocation(line: 45, column: 1, scope: [[DBG5]])
; CHECK: [[DBG10]] = !DILocation(line: 46, column: 1, scope: [[DBG5]])
; CHECK: [[DBG11]] = !DILocation(line: 47, column: 1, scope: [[DBG5]])
; CHECK: [[DBG12]] = !DILocation(line: 44, column: 1, scope: [[DBG5]])
;.
