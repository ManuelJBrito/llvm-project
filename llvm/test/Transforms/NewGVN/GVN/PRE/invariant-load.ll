...
; Test if the !invariant.load metadata is maintained by newgvn.
...
; CHECK-NEXT:    br i1 [[CND]], label %[[BB1:.*]], label %[[ENTRY_BB2_CRIT_EDGE:.*]]
...
; CHECK-NEXT:    [[RES_PRE:%.*]] = sub i32 [[V1]], [[V2_PRE]]
; CHECK-NEXT:    br label %[[BB2]]
; CHECK:       [[BB2]]:
; CHECK-NEXT:    [[PHIOFOPS1:%.*]] = phi i32 [ [[V1]], %[[ENTRY_BB2_CRIT_EDGE]] ], [ [[V2_PRE]], %[[BB1]] ]
; CHECK-NEXT:    [[RES:%.*]] = phi i32 [ 0, %[[ENTRY_BB2_CRIT_EDGE]] ], [ [[RES_PRE]], %[[BB1]] ]
; CHECK-NEXT:    ret i32 [[RES]]
;
; %v2 is redundant, but newgvn currently doesn't catch that
entry:
...
; CHECK-NEXT:  [[ENTRY:.*:]]
; CHECK-NEXT:    [[V1:%.*]] = load i32, ptr [[P]], align 4, !invariant.load [[META0]]
; CHECK-NEXT:    br i1 [[CND]], label %[[TAKEN:.*]], label %[[ENTRY_MERGE_CRIT_EDGE:.*]]
; CHECK:       [[ENTRY_MERGE_CRIT_EDGE]]:
; CHECK-NEXT:    br label %[[MERGE:.*]]
; CHECK:       [[TAKEN]]:
; CHECK-NEXT:    [[P2:%.*]] = call ptr (...) @bar(ptr [[P]])
; CHECK-NEXT:    [[V2:%.*]] = load i32, ptr [[P2]], align 4, !invariant.load [[META0]]
; CHECK-NEXT:    [[RES:%.*]] = sub i32 [[V1]], [[V2]]
; CHECK-NEXT:    br label %[[MERGE]]
; CHECK:       [[MERGE]]:
; CHECK-NEXT:    [[PHIOFOPS1:%.*]] = phi i32 [ [[V1]], %[[ENTRY_MERGE_CRIT_EDGE]] ], [ [[V2]], %[[TAKEN]] ]
; CHECK-NEXT:    [[PHIOFOPS:%.*]] = phi i32 [ 0, %[[ENTRY_MERGE_CRIT_EDGE]] ], [ [[RES]], %[[TAKEN]] ]
; CHECK-NEXT:    [[P3:%.*]] = phi ptr [ [[P]], %[[ENTRY_MERGE_CRIT_EDGE]] ], [ [[P2]], %[[TAKEN]] ]
