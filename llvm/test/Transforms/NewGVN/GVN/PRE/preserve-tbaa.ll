...
; newgvn should preserve the TBAA tag on loads when doing PRE.

define void @test(ptr %P, ptr %Q, i1 %arg) nounwind {
; CHECK-LABEL: define void @test(
; CHECK-SAME: ptr [[P:%.*]], ptr [[Q:%.*]], i1 [[ARG:%.*]]) #[[ATTR0:[0-9]+]] {
; CHECK-NEXT:  [[ENTRY:.*:]]
; CHECK-NEXT:    br i1 [[ARG]], label %[[BB_NPH:.*]], label %[[ENTRY_FOR_END_CRIT_EDGE:.*]]
; CHECK:       [[ENTRY_FOR_END_CRIT_EDGE]]:
; CHECK-NEXT:    br label %[[FOR_END:.*]]
; CHECK:       [[BB_NPH]]:
; CHECK-NEXT:    [[TMP33_PRE:%.*]] = load i16, ptr [[P]], align 2, !tbaa [[TBAA0:![0-9]+]]
; CHECK-NEXT:    br label %[[FOR_BODY:.*]]
; CHECK:       [[FOR_BODY]]:
; CHECK-NEXT:    [[TMP33:%.*]] = phi i16 [ [[TMP33_PRE]], %[[BB_NPH]] ], [ 0, %[[FOR_BODY_FOR_BODY_CRIT_EDGE:.*]] ]
; CHECK-NEXT:    store i16 [[TMP33]], ptr [[Q]], align 2
; CHECK-NEXT:    store i16 0, ptr [[P]], align 2, !tbaa [[TBAA0]]
; CHECK-NEXT:    br i1 false, label %[[FOR_BODY_FOR_END_CRIT_EDGE:.*]], label %[[FOR_BODY_FOR_BODY_CRIT_EDGE]]
