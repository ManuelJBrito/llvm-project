...
; NewGVN handles store-to-load and redundant load elimination across
; @llvm.sideeffect, same as GVN. But NewGVN also does loop-invariant
; load hoisting (unlike GVN which uses a separate PRE mechanism).
...
; CHECK-NEXT:    call void @llvm.sideeffect()
; CHECK-NEXT:    [[T3:%.*]] = load float, ptr [[P]], align 4
; CHECK-NEXT:    [[T4]] = fadd float [[SUM]], [[T3]]
