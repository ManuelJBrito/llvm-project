...
; XFAIL: *
; RUN: opt < %s -passes=newgvn -disable-output -pass-remarks=newgvn -pass-remarks-missed=newgvn  \
; RUN:     2>&1 | FileCheck %s
; RUN: opt < %s -passes=newgvn -disable-output -pass-remarks-output=%t -S
; RUN: cat %t | FileCheck -check-prefix=YAML %s

; RUN: opt < %s -aa-pipeline=basic-aa -passes=newgvn -disable-output -pass-remarks=newgvn -pass-remarks-missed=newgvn \
; RUN:     2>&1 | FileCheck %s
; RUN: opt < %s -aa-pipeline=basic-aa -passes=newgvn -disable-output -pass-remarks-output=%t -S
; RUN: cat %t | FileCheck -check-prefix=YAML %s
