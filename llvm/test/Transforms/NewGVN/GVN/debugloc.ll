; RUN: opt < %s -passes=newgvn -S -disable-output
; NewGVN does not perform PRE, so the load inside the loop stays.
; GVN would hoist the load and create PHI nodes without debug locations.
; This test just verifies NewGVN doesn't crash on debug info.
