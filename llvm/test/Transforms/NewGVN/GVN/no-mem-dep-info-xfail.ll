...
; RUN: opt %s -passes=newgvn -S -enable-newgvn-memdep=false | FileCheck %s
; RUN: opt %s -passes=newgvn -S -enable-newgvn-memdep=true | FileCheck %s

; Check that llvm.x86.avx2.gather.d.ps.256 intrinsic is not eliminated by newgvn
